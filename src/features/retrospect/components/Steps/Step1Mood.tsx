    ...
    <StepContainer>
      ...
      <div className="grid grid-cols-5 gap-3">
        {MOODS.map(({ emoji, mood }) => (
          <button
            key={emoji}
            className={`flex items-center justify-center text-2xl w-12 h-12 border border-gray-300 rounded-lg transition ${
              selectedMood === emoji ? "bg-gray-300 border-gray-500" : "bg-white"
            }`}
